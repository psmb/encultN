  ...
  render() {
    return (
      <Link key={this.props.question.get('id')} className='mdl-cell QuestionSmall' to={`/q/${this.props.question.get('id')}`}>
        ...
        <p className='mdl-typography--caption-color-contrast QuestionSmall-lead'>Всего голосов: <span className='color-accent'>{this.props.question.get('voteCount')}</span> • Лидирует <span className='color-accent'><strong>{this.props.question.get('worldviewWithMaxVotes')} ({this.props.question.get('answerWithMaxVotesCount')})</strong></span></p>
      </Link>
    );
  }
}
